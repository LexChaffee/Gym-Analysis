...
Let's take a step back from Frequency as a predictor variable and start doing multivariate analysis with multiple response and predictor variables using the cbind: 
...
```
So based off the output, it appears that ExpLvl, Gender, and WWater Intake have a significant predictive effect on the response variables.
...
mlm1 <- lm(cbind(Freq, Duration, CaloriesBurned) ~ ExpLvl + Gender + WorkoutType + WaterIntake, data = gym_data)
summary(mlm1)
```
...
For the response variable Duration it looks like ExpLvl, Gender, and Water Intake have a significant affect on our response variable, with Duration increasing by 0.336064 as ExpLvl increases by one unit, -0.069584 if the sex is male, and 0.074597 as water increases by one unit

For the response variable Calories it looks like ExpLvl, Gender, and Water Intake  have a significant affect on our response variable, with Calories increasing by 244.798 as ExpLvl increases by one unit, 45.251 if the sex is male, and 45.385 as water increases by one unit.
